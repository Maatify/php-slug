...
final class ResultSnapshotDecoder
{
    ...
    private static function historyList(mixed $value, string $path, SlugProfileInterface $profile, ?array $bindingIds = null): array
    {
        ...
        if ($bindingIds === null) {
            self::orderedHistory($events, $path);
        } else {
            self::participantHistory($events, $bindingIds, $path);
        }
        return $events;
    }
    ...
    private static function participantHistory(array $items, array $bindingIds, string $field): void
    {
        ...
        foreach ($items as $item) {
            ...
            if ($participant === false || $participant < $lastParticipant) {
                self::fail(sprintf('%s has invalid participant ordering.', $field));
            }
            ...
            if (isset($lastSequence[$item->bindingId]) && ($item->sequenceNo < $lastSequence[$item->bindingId] || ($item->sequenceNo === $lastSequence[$item->bindingId] && $item->id <= $lastId[$item->bindingId]))) {
                self::fail(sprintf('%s must be ordered within each participant.', $field));
            }
    ...
    private static function dateRange(DateTimeImmutable $value, string $field): void
    {
        ...
        if ($utc < $min || $utc > $max) {
            self::fail(sprintf('%s is outside the supported UTC range.', $field));
        }
    }
